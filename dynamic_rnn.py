...
class DynamicLSTM(nn.Module):
    ...
    def forward(self, x, x_len, h0=None):
        ...
        if self.only_use_last_hidden_state:
            ...
            """unpack: out"""
            out = torch.nn.utils.rnn.pad_packed_sequence(out_pack, batch_first=self.batch_first, total_length=L)  # (sequence, lengths)
            out = out[0]  #
            out = out[x_unsort_idx]
            # out_forward = out[:, :, :self.hidden_size]
            # out_backward = out[:, :, -self.hidden_size:]
